...
public class JournalPanel : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (openButtonGroup != null)
            openButtonGroup.SetActive(GoBagFloater.IsBagPickedUp());
    }

    void Update()
    {
        // The Journal only becomes available once the player has picked up the go bag
        if (openButtonGroup != null && !openButtonGroup.activeSelf && GoBagFloater.IsBagPickedUp())
    ...
    public void Open()
    {
        if (isBusy || overlay == null || overlay.activeSelf || !GoBagFloater.IsBagPickedUp())
            return;
