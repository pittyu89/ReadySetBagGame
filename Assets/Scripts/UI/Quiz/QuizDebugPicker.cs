...
// reach players: QuizManager's matching override is behind the same guard.
...
/// in what order.
///
/// A round is as long as the session's difficulty asks for (see QuizManager), so the choice
...
public class QuizDebugPicker : MonoBehaviour
{
    ...
    {
        get { return Mathf.Min(QuizManager.CurrentQuestionsPerRound, questionCount); }
    }
    ...
    private void ApplySelection()
    {
        ...
        QuizManager.DebugQuestionSubset = IsComplete ? new List<int>(picked) : null;
